  ...
  with monitored_session.MonitoredSession(
    ...
    if eval_ops is not None:
      while not session.should_stop():
        ...
        i += 1
  if total_iters > 0:
    throughput = total_iters / total_time
  else:
    throughput = 0

  print('Throughput: {:0.5f} steps/s'.format(throughput))
  print('Inference Time : {:0.5f}s'.format(time.time() - start))

  print('Finished evaluation at ' +
